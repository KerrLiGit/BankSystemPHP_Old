<?php
	...
	function create_account($idclient, $currency, $acc2p, $descript, &$res_account = NULL, $pmysqli = NULL) {
		...
		$stmt = $mysqli->prepare("SELECT count(*) FROM account WHERE idclient = ? AND (closed = '0000-00-00' OR closed IS NULL) AND currency = ?");
	...
	function find_bank_account($accountnum, $mask, &$corraccountnum, $pmysqli = NULL) {
		...
		$stmt = $mysqli->prepare(
			"SELECT accountnum " .
			"FROM account " .
			"WHERE idclient = 1 AND accountnum LIKE ? AND (closed = '0000-00-00' OR closed IS NULL) " .
	...
	function out_account_box($idclient, $descript = "", $out_null = false) { // вывод списка счетов 40817 клиента
		...
		$stmt = $mysqli->prepare(
			...
			"WHERE (closed = '0000-00-00' OR closed IS NULL) AND a.idclient = ? AND accountnum LIKE '40817%'");
		...
		if ($descript == "out_acc") {
			$stmt = $mysqli->prepare(
				...
				"  LEFT JOIN currency c ON c.code = a.currency " . 
				"  LEFT JOIN accounttype t ON t.acc2p = SUBSTR(a.accountnum, 1, 5) " .
				"WHERE (closed = '0000-00-00' OR closed IS NULL) AND idclient = ?");
		}
	...
	function client_account_count_any($idclient) { // кол-во действующих счетов 40817 (c любым остатком) у клиента
		$mysqli = get_sql_connection();
		$stmt = $mysqli->prepare("SELECT COUNT(*) FROM account WHERE idclient = ? AND (closed = '0000-00-00' OR closed IS NULL) AND accountnum LIKE '40817%'");
		$stmt->bind_param("i", $idclient);
		$stmt->execute();
	...
	function client_account_count($idclient, $zeroonly) { // кол-во действующих счетов 40817 [с нулевым остатком] у клиента
		...
		// только без остатка
		$mysqli = get_sql_connection();
		$stmt = $mysqli->prepare("SELECT accountnum FROM account WHERE idclient = ? AND (closed = '0000-00-00' OR closed IS NULL) AND accountnum LIKE '40817%'");
		$stmt->bind_param("i", $idclient);
		$stmt->execute();
		$res = $stmt->get_result();
		$accounts = array();
